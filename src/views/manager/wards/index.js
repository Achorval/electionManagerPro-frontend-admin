...
// ** Store & Actions
import { getLgas, getWards, deleteWard } from '../store';
...
const LgaList = () => {
  ...
  const lgaSchema = yup.object().shape({
    lgaId: yup.string().required('Lga is a required field'),
  ...
  const onSubmit = async data => {
    try {
      if (selected === 'Add') {
        await useAxios.post('/api/admin/wards/create', {
          lgaId: data.lgaId,
          name: data.name
        });
        reset();
        ...
        await useAxios.put(`/api/admin/wards/update/${state.id}`, {
          lgaId: data.lgaId,
          name: data.name
        });
        reset();
  ...
  return (
    <div className='invoice-list-wrapper'>
      <h2 className='fw-bolder mb-75'>Wards</h2>
      <Card className='p-0'>
        <CardHeader>
          <div className='w-100'>
        ...
        <CardBody className='p-2'>
          {isLoading ? (
            <FallbackSpinner />
          ) : store && store.wards && store.wards.length > 0 ? (<>
            <Table hover responsive bordered>
              ...
              <tbody className='text-center'>
                {store.wards.map((value, index) => {
                  return (
                    <tr key={index}>
                      <td><span className='fw-bold'>{sn++}</span></td>
                      <td><span className='fw-bold'>{value.lga.name}</span></td>
            ...
            <div className='text-center p-3 p-lg-5'>
              <img src={empty} />
              <h4 className='alert-heading mt-5'>You do not have any wards list yet</h4>
            </div>
          )}
        </CardBody>
      </Card>
      ...
      {show && (
        <Modal isOpen={show} toggle={onModalClosed} className='modal-dialog-centered' onClosed={onModalClosed}>
          ...
          <ModalBody className='px-sm-2 mx-50 pb-3'>
            <h1 className='text-center'>{selected === 'Add' ? 'Add New' : 'Edit'} Ward</h1>
            <Form className='mt-2' onSubmit={handleSubmit(onSubmit)}>
              <div className='mb-1'>
                <Label className='form-label' for='lgaId'>
                  LGA
                </Label>
                <Controller
                  id='lgaId'
                  ...
                  control={control}
                  render={({ field }) => (
                    <Input type='select' className='py-1' invalid={errors.lgaId && true} {...field}>
                      <option value=''>select...</option>
                      {store.lgas.map(n => <option key={n.id} value={n.id}>{n.name}</option>)}
                    </Input>
                  )}
                />
                {errors.lgaId && <FormFeedback>{errors.lgaId.message}</FormFeedback>}
              </div>
              <div className='mb-1'>
              ...
              <Button disabled={isSubmitting} type='submit' className='p-1' color='primary' block>
                Add Ward {isSubmitting && <Spinner color='white' size='sm' />}
              </Button>
            </Form>
          </ModalBody>
        </Modal>
