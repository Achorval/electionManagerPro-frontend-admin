...
// ** Store & Actions
import { getStates, getLgas, deleteLga } from '../store';
...
const LgaList = () => {
  ...
  useEffect(() => {
    ...
    // hideLoader()
  }, [dispatch, store && store.lgas.length]);
  ...
  const lgaSchema = yup.object().shape({
    stateId: yup.string().required('State is a required field'),
  ...
  const onSubmit = async data => {
    try {
      if (selected === 'Add') {
        await useAxios.post('/api/admin/lgas/create', {
        ...
        await useAxios.put(`/api/admin/lgas/update/${state.id}`, {
          stateId: data.stateId,
          name: data.name
        });
        reset();
  ...
  const handleConfirmCancel = (id) => {
    return MySwal.fire({
      ...
      if (result.value) {
        dispatch(deleteLga(id))
        MySwal.fire({
          icon: 'success',
          title: 'Deleted!',
          text: 'Lga has been deleted.',
          customClass: {
  ...
  return (
    <div className='invoice-list-wrapper'>
      <h2 className='fw-bolder mb-75'>Lgas</h2>
      <Card className='p-0'>
        <CardHeader>
          <div className='w-100'>
        ...
        <CardBody className='p-2'>
          {isLoading ? (
            <FallbackSpinner />
          ) : store && store.lgas && store.lgas.length > 0 ? (<>
            <Table hover responsive bordered>
              ...
              <tbody className='text-center'>
                {store.lgas.map((value, index) => {
                  return (
                    <tr key={index}>
                      <td><span className='fw-bold'>{sn++}</span></td>
                      <td><span className='fw-bold'>{value.state.name}</span></td>
            ...
            <div className='text-center p-3 p-lg-5'>
              <img src={empty} />
              <h4 className='alert-heading mt-5'>You do not have any lgas list yet</h4>
            </div>
          )}
        </CardBody>
      </Card>
      ...
      {show && (
        <Modal isOpen={show} toggle={onModalClosed} className='modal-dialog-centered' onClosed={onModalClosed}>
          ...
          <ModalBody className='px-sm-2 mx-50 pb-3'>
            <h1 className='text-center'>{selected === 'Add' ? 'Add New' : 'Edit'} LGA</h1>
            ...
            <div className='mb-1'>
              ...
              <Controller
                ...
                control={control}
                render={({ field }) => (
                  <Input type='select' className='py-1' invalid={errors.stateId && true} {...field}>
                    <option value=''>select...</option>
                    {store.states.map(n => <option key={n.id} value={n.id}>{n.name}</option>)}
                  </Input>
                )}
              />
              {errors.stateId && <FormFeedback>{errors.stateId.message}</FormFeedback>}
            </div>
              <div className='mb-1'>
              ...
              <Button disabled={isSubmitting} type='submit' className='p-1' color='primary' block>
                Add Lga {isSubmitting && <Spinner color='white' size='sm' />}
              </Button>
            </Form>
          </ModalBody>
        </Modal>
